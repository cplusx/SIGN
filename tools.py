...
def construct_gt_st(test_res, config):
    resized_gt_st = test_res['resized_gt'].cpu() # torch.LongTensor with shape (N,H',W'). elements: 0-14,255 in voc12
    sorted_indices = test_res['sorted_indices'].cpu() # torch.LongTensor with shape (N,H',W',C)
    cls_score = test_res['cls_score'].cpu()
    return construct_gt_st_softmax(resized_gt_st, sorted_indices, cls_score, config)



def construct_gt_st_softmax(resized_gt_st, sorted_indices, cls_score, config):
    # create gt_new
    dim = sorted_indices.shape[-1]
    indices_select = sorted_indices
    ...
    indices_repeat = torch.LongTensor(range(dim)).repeat(indices_select.shape[0],indices_select.shape[1],indices_select.shape[2],1)
    ...
    accumulated = dim * torch.LongTensor(range(p2.shape[0]*p2.shape[1]*p2.shape[2]))
    ...
    # create gt_loss_weight
    bs, h, w, dim = cls_score.shape
    cls_score = cls_score.view(bs, h*w, dim)
    conf_score = torch.softmax(cls_score, dim=-1)

    ignore_region = (resized_gt_st == config['ignore_index']).view(bs, h*w)
    conf_score = conf_score.view(-1)[p5].view(bs, h*w)
    conf_score[ignore_region] = 0
    conf_score = torch.where(ignore_region, conf_score, torch.zeros_like(conf_score))
    conf_score_ = torch.softmax(config['T']*conf_score, dim=1) # config['T'] is the temperature used in softmax
    conf_score_ /= (torch.max(conf_score_, dim=1, keepdim=True)[0] + 1e-6)
    conf_score_[conf_score_ != conf_score_] = 0. # convert nan to 0
    conf_score_ = conf_score_.view(bs, h, w)
    ignore_region = ignore_region.view(bs, h, w)
    gt_loss_weight = torch.where(ignore_region, conf_score_, torch.ones_like(conf_score_))
    return gt_new, gt_loss_weight
