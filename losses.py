...
class ClsLoss(nn.Module):
    def __init__(self, opt):
        ...
        self.criterionCLS = nn.CrossEntropyLoss(ignore_index=opt['ignore_index'], reduction='none')

    def forward(self, net, feat, gt, ignore_mask, gt_loss_weight):
        cls_score = net(feat, 'cls')
        loss_cls_before_reduction = self.criterionCLS(cls_score, gt)
        gt_loss_weight = gt_loss_weight.to(loss_cls_before_reduction.device)
        loss_cls = torch.sum(loss_cls_before_reduction * gt_loss_weight) / (torch.sum(gt_loss_weight) + 1e-6)
        ...
        return loss_cls, acc_cls, pred_class, indices_, cls_score.detach().permute(0,2,3,1).contiguous()
