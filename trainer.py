...
class Trainer(nn.Module):
    def __init__(self, cfg, class_emb_vis, class_emb_all, schedule, checkpoint_dir=None, resume_from=None, device='cuda'):
        super(Trainer, self).__init__()
        assert (schedule in ['step1', 'mixed', 'st', 'st_mixed'])

        self.cfg = cfg
        self.schedule = schedule
        self.model = OurModel(cfg, class_emb_vis, class_emb_all).to(device)
    ...
    def train(self, data, gt, mode, multigpus, gt_loss_weight):
        assert (mode == 'step1' or mode == 'step2')

        if mode == 'step1':
            flag, loss_G_GAN, loss_G_Content, loss_G_cls, loss_G, loss_B_KLD, loss_B_cls, loss_B, loss_D_real, loss_D_fake, loss_D_gp, loss_D_GAN, loss_D_cls_real, loss_D_cls_fake, loss_D_cls, loss_D, loss_cls_real, loss_cls_fake, acc_cls_real, acc_cls_fake = self.model(data, gt, mode, gt_loss_weight)
        else:
            flag, loss_G_cls, loss_G, loss_D_cls_fake, loss_D_cls, loss_D, loss_cls_fake, acc_cls_fake = self.model(data, gt, mode, gt_loss_weight)
        if (flag == -1).any() == True:
    ...
    def test(self, data, gt, multigpus):
        with torch.no_grad():
            ...
            flag, pred_cls_real, sorted_indices, resized_gt, resized_ignore_mask, cls_score = this_model.test(data, gt)
            if (flag == -1).any() == True:
                raise MeaninglessError()
    ...
    def qualitative_test(self, data, gt, multigpus):
        with torch.no_grad():
            this_model = self.model.module if multigpus else self.model

            flag, pred_cls_real, sorted_indices, resized_gt, resized_ignore_mask, cls_score = this_model.qualitative_test(data, gt)
            if (flag == -1).any() == True:
                raise MeaninglessError()
    ...
    def resume(self, checkpoint_dir, resume_from):
        ...
        self.back_opt.load_state_dict(state_dict['back_opt'])
        ...
        torch.save(
            {
                ...
                'back_opt': self.back_opt.state_dict(),
...
def get_params(model, key):
    ...
    if key == "1x":
        for m in model.named_modules():
            if "layer" in m[0] or "vgg" in m[0] or "fc_" in m[0]:
    ...
    if key == "5x":
        for m in model.named_modules():
            if "contextual" in m[0] or "positional" in m[0]:
                if isinstance(m[1], nn.Conv2d):
                    for p in m[1].parameters():
                        yield p
