...
def main():
    ...
    if args.val:
        save_path = 'Results/{}'.format(args.experimentid)
    ...
    loader_test = torch.utils.data.DataLoader(
        ...
        batch_size=1, #config['BATCH_SIZE']['TEST'],
        num_workers=config['NUM_WORKERS'],
        shuffle=False
    )
    ...
    if args.val:
        ...
        Only do validation
        """
        from test_tools import mean_iou_of_index, has_unseen_index, harmonic_mean, acc_of_index
        reverse_cls_map_test = np.array([255] * len(cls_map_test))
        for i in np.unique(cls_map_test):
            if i == 255: continue
            reverse_cls_map_test[i] = cls_map_test.tolist().index(i)
        ...
        while True:
            ...
            if 'short_snapshot' in config and (iteration + 1) % config['snapshot'] == 0:
