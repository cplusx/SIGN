# self training with separate model
...
from tools import get_embedding, get_split, get_config, logWritter, MeaninglessError, scores_gzsl, Step_Scheduler, Const_Scheduler, construct_gt_st, resize_data
...
def parse_args():
    ...
    parser.add_argument('--auto_resume', type=int, default=0, help='set to 1 to auto resume from directory')
...
def main():
    ...
    assert (not (args.val and args.auto_resume > 0))
    ...
    if args.resume_from >= 1:
        ...
        if not os.path.exists(config_path):
            ...
            if config['save_path'] != config_old['save_path'] or config['dataset'] != config_old['dataset']:
                assert 0, "New config does not coordinate with old config."
            config = config_old
            complete_models = sorted(glob.glob(os.path.join(model_path, "*.pth")))
            if len(complete_models) > 0:
                start_iter = int(os.path.basename(complete_models[-1]).replace('.pth', ''))
                if args.schedule == 'step1':
                ...
                if start_iter >= ITER_MAX:
                    print('Training is complete, exit now')
                    sys.exit(0)
                print("Train from scratch ...")
                logger.write("Train from scratch ...")
            else:
    ...
    st_trainer = Trainer(
        ...
        schedule=args.schedule, 
        checkpoint_dir=model_path,  # for model loading in continued train
        resume_from=0,  # for model loading in continued train
        device=device
    )
    st_trainer.model.eval() # only use st_trainer for getting self training gt
    if args.multigpus:
        trainer.model = torch.nn.DataParallel(trainer.model, device_ids=range(args.ngpu))
        st_trainer.model = torch.nn.DataParallel(st_trainer.model, device_ids=range(args.ngpu))
    ...
    if args.val:
        ...
        while True:
            ...
            if mode == 'step1' or mode == 'step2':
                try:
                    gt_loss_weight = torch.where(gt == config['ignore_index'], torch.zeros_like(gt), torch.ones_like(gt))
                    loss = trainer.train(data, gt, mode=mode, multigpus=args.multigpus, gt_loss_weight=gt_loss_weight)
                except MeaninglessError:
                ...
                try:
                    with torch.no_grad():
                        test_res = st_trainer.test(data, gt, multigpus=args.multigpus) # EDIT HERE: trainer -> st_trainer
                        gt_new = construct_gt_st(test_res, config)

                    gt_new, gt_loss_weight = gt_new
                    # gt_loss_weight = torch.where(resize_data(data.sum(1), gt_loss_weight.shape[2]) == 0., torch.zeros_like(gt_loss_weight), gt_loss_weight) # make border loss weight to 0
                    print('{} percent non zero'.format(gt_loss_weight.sum().type(torch.float32) / (gt_loss_weight.shape[0] * gt_loss_weight.shape[1] * gt_loss_weight.shape[2])))
                    loss = trainer.train(data, gt_new, mode='step1', multigpus=args.multigpus, gt_loss_weight=gt_loss_weight)
                except MeaninglessError:
                    print("Skipping meaningless batch...")
                    continue
            ...
            done_iters = iteration+1 - start_iter
            time_per_iter = (current_time - start_time) / done_iters
            remaining_time = (ITER_MAX - iteration) * time_per_iter
            remaining_H, remaining_M, remaining_S = int(remaining_time//3600), int((remaining_time%3600)//60), remaining_time%3600%60
            print(f'Iter {iteration}/{ITER_MAX}, {time_per_iter} secs/iter, ETA: {remaining_H}:{remaining_M}:{remaining_S:.1f}')
